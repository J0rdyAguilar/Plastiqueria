...
class PedidoController extends Controller
{
    ...
    public function aprobarMontoVariable(Pedido $pedido, Request $request)
    {
        ...
        $tieneMontoVariable = $pedido->detalles()
            ->where('es_monto_variable', 1)
            ->exists();

        if (!$tieneMontoVariable) {
            return response()->json([
                'message' => 'Este pedido no tiene monto variable.'
            ], 422);
        }

        if ($pedido->monto_variable_estado === 'aprobado') {
            return response()->json([
                'message' => 'Este monto variable ya fue aprobado.'
            ], 422);
        }

        if ($pedido->monto_variable_estado === 'rechazado') {
            return response()->json([
                'message' => 'Este monto variable ya fue rechazado.'
            ], 422);
        }
